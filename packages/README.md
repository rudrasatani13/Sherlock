...
- `pdf_export`: Phase 19 PDF Report Export foundation for export contracts, safety checks, print-ready HTML rendering, and local/demo artifact generation
...
The scanner engine, prompt library, evaluator system, worker system, scan limits, findings system, report system, and PDF export package are internal only. They do not implement backend scan execution APIs, production auth, active database persistence, billing, production dashboard integration, production PDF delivery, public PDF download links, report generation from real scans, active findings persistence, real customer evidence storage, real report sharing, or public scanning.

Phase 9 adds a separate FastAPI backend foundation under `../apps/api`. Phase 17 exposes only static findings schema metadata. Phase 18 exposes only static report schema metadata and a static dashboard report shell. Phase 19 extends the reports schema metadata with a static PDF export contract. It does not expose scanner execution, evaluator execution, findings persistence, customer report retrieval, report database writes, public PDF download links, production storage, sharing tokens, or customer evidence through public HTTP routes.
