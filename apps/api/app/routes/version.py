...
def _module_statuses(api_prefix: str) -> list[ModuleStatus]:
    return [
        ...
        ModuleStatus(
            module="reports",
            status="web_report_foundation",
            purpose="Phase 18 web report schema metadata and report access placeholder.",
            future_phase="Phase 19 PDF export after web report foundation",
            available_endpoints=[f"GET {api_prefix}/reports", f"GET {api_prefix}/reports/schema"],
            future_capabilities=["authenticated report access", "report persistence", "report sharing controls", "PDF export"],
            disabled_capabilities=[
