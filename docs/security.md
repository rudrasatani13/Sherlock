...
- Phase 19 PDF export objects must use only redacted, short, report-safe evidence. The export safety checker blocks raw Authorization headers, raw cookies, API keys, bearer tokens, private keys, full private document markers, large snippets, unsafe filenames, missing limitations, and overclaiming verdict language.
- Generated PDF/HTML report export artifacts must stay in ignored directories such as `pdf-output/`, `pdf-exports/`, or `report-exports/` and must not be committed.
...
- Phase 19 PDF exports must reject overclaiming verdict text before rendering or writing local artifacts.
